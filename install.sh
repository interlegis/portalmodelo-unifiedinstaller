...
if test `whoami` != "root"; then
    ...
    PLONE_HOME='/var/interlegis/PortalModelo-3.0'
...
LOCAL_HOME="$HOME/PortalModelo-3.0"
...
# Create a PLONE HOME directory for install Portal Modelo
if test `whoami` != "root"; then
    PLONE_HOME=`pwd`
else
    PLONE_HOME='/var/interlegis/PortalModelo-3.0'
...
if [ "X$WITH_PYTHON" != "X" ] && [ "X$HAVE_PYTHON" = "Xno" ]; then
    ...
    "$WITH_PYTHON" virtualenv.py --no-setuptools --no-site-packages "$PY_HOME"
    cd "$PKG"
    rm -r $VIRTUALENV_DIR
